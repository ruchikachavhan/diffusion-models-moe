...
# read all the files
files = os.listdir(os.path.join(res_path, model_name, folder_name))
files = [f for f in files if f.endswith('.json')]
print(files)
num_time_steps = 51

for f in files:
    with open(os.path.join(res_path, model_name, folder_name, f), 'r') as file:
        data = json.load(file)
        timesteps = data.keys()
        ...
        if not os.path.exists(os.path.join(res_path, model_name, folder_name, f.split(".json")[0])):
            os.makedirs(os.path.join(res_path, model_name, folder_name, f.split(".json")[0]))
        ...
        for t in timesteps:
            ...
            plt.savefig(os.path.join(res_path, model_name, folder_name, f.split(".json")[0], f'freq_{t}.png'))
            plt.close()
