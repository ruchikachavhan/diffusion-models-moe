...
def main():
    ...
    args.modularity['skill_type'] = 't_test'
    ...
    for i in range(1, len(base_prompts)+1):
        ...
        base_states = torch.load(os.path.join(args.save_path, 'hidden_states', f'hidden_states_base_{i}.pth'))
        adj_states = torch.load(os.path.join(args.save_path, 'hidden_states',  f'hidden_states_adj_{i}.pth'))
    ...
    for t in range(args.timesteps):
        ...
        for l in range(args.n_layers):
            ...
            with open(os.path.join(args.modularity['skilled_experts_t_test'], f'timestep_{t}_layer_{l}.json'), 'w') as f:
