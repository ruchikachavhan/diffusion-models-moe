...
def main():
    ...
    for name, module in model.text_encoder.named_modules():
            if isinstance(module, args.replace_fn) and 'mlp' in name and 'encoder.layers' in name:
                ...
                weight = module.fc1.weight.detach().clone()
    ...
    if not os.path.exists(os.path.join(args.modularity['skill_neuron_path'], 'base_norms.pt')):
        ...
        torch.save(act_norms_base, os.path.join(args.modularity['skill_neuron_path'], 'base_norms.pt'))
        torch.save(act_norms_adj, os.path.join(args.modularity['skill_neuron_path'], 'adj_norms.pt'))
    else:
    ...
    for l in range(num_geglu):
        ...
        pruned_indx = sorted_idx[:, :int(0.5 * metric_base.shape[1])].numpy()
        ...
        pruned_indx_adj = sorted_idx_adj[:, :int(0.5 * metric_adj.shape[1])].numpy()

        # check the set diffferemce between pruned indices
        pruned_indx = [set(i) for i in pruned_indx]
        pruned_indx_adj = [set(i) for i in pruned_indx_adj]

        # import pdb; pdb.set_trace()
        for i in range(len(pruned_indx)):
            set_diff = pruned_indx_adj[i].difference(pruned_indx[i])
            print("Set diff: ", set_diff)
            binary_mask_adj[i, list(set_diff)] = 1
